...
Use the shadcn MCP tools, the project's `components.json`, and the official shadcn/ui docs as the primary references for setup, component discovery, customization, and migration guidance.

## Examples

When available, prefer examples already present in the host project. Otherwise, use `get_component_demo` and related MCP discovery calls to inspect canonical component usage patterns before integrating.
